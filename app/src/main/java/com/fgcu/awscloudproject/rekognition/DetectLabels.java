...
public class DetectLabels extends AppCompatActivity {
  ...
  public DetectLabels(String photoName, Context context, DynamoDBMapper dynamoDBMapper,
      String mCurrentPhotoPath, ProgressBar progressBar) {
  ...
  private void init() {
    ...
    AmazonRekognitionClient amazonRekognitionClient =
        new AmazonRekognitionClient(credentialsProvider);
  ...
  private class AsyncTaskRunner extends AsyncTask<Void, Void, Void> {
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      Log.d("DataModelResponse", photoName + "\t" + dataModelResponse);
      //Upload to specified folder
      ReUploadToS3 reUploadToS3 =
          new ReUploadToS3(photoName, mCurrentPhotoPath, context, dynamoDBMapper);
      switch (dataModelResponse) {
      ...
      //Write to database
      UploadToTable uploadToDatabase =
          new UploadToTable(dynamoDBMapper, photoName, dataModelResponse);
